<template>
  <!-- 弹窗对话框，根据mode的不同显示不同的标题，支持新增、编辑和查看操作 -->
  <t-drawer
    :visible="dialogVisible"
    :header="
      mode == 'new'
        ? '新增'
        : mode == 'edit'
          ? '编辑'
          : mode == 'view'
            ? '查看'
            : ''
    "
    @confirm="handleSubmit"
    @cancel="handleClose"
    @close="handleClose"
    size="40%"
  >
    <!-- 表单，用于输入用户信息，密码字段为密码类型 -->
    <t-form
      ref="formInstance"
      :data="pageData.formData"
      :rules="pageData.formRules"
      :disabled="pageData.readOnly"
    >
      <t-form-item label="角色名称" name="roleName">
        <t-input v-model="pageData.formData.roleName" />
      </t-form-item>
      <t-form-item label="角色类型" name="codeType">
        <t-select
          v-model="pageData.formData.codeType"
          @change="handleCodeTypeChange"
        >
          <t-option label="超级管理员" value="root" />
          <t-option label="管理员" value="admin" />
          <t-option label="普通用户" value="user" />
          <t-option label="游客" value="visitor" />
          <t-option label="自定义" value="custom" />
        </t-select>
      </t-form-item>
      <t-form-item label="角色代号" name="id">
        <t-input
          v-model="pageData.formData.id"
          :disabled="pageData.formData.codeType !== 'custom'"
        />
      </t-form-item>
      <t-form-item label="备注" name="remark">
        <t-textarea v-model="pageData.formData.remark" />
      </t-form-item>
    </t-form>
  </t-drawer>
</template>

<script lang="ts" setup>
import { reactive, ref } from "vue";
import { http } from "@/utils/fetch";
// import { v4 as uuidv4 } from "uuid";

const props = defineProps(["id", "dialogVisible", "mode"]);
const emits = defineEmits(["close"]);

import { useUserStore } from "@/stores/modules/user";
import {
  MessagePlugin,
  type FormInstanceFunctions,
  type FormProps
} from "tdesign-vue-next";

const formInstance = ref<FormInstanceFunctions | null>(null);

const GET_FORM_PATH = "/user/role/findOne"; // 查询接口
const CREATE_FORM_PATH = "/user/role/create"; // 创建接口
const UPDATE_FORM_PATH = "/user/role/update"; // 更新接口

const userStore = useUserStore();

const pageData = reactive<Record<string, any>>({
  readOnly: false,
  formData: {
    id: "",
    createdTime: "",
    updatedTime: "",
    createdBy: "",
    updatedBy: "",
    remark: "",
    roleName: "",
    codeType: ""
  },
  formRules: {
    roleName: [
      {
        required: true,
        message: "请输入角色名称",
        type: "error",
        trigger: "blur"
      }
    ],
    codeType: [
      {
        required: true,
        message: "请指定角色类型",
        type: "error",
        trigger: "blur"
      }
    ],
    id: [
      {
        required: true,
        message: "请输入角色代号",
        type: "error",
        trigger: "blur"
      }
    ]
  }
});

/**
 * 初始化表单数据。
 * 根据不同的操作模式（新增、编辑、查看），加载不同的数据。
 * @param id - ID，用于编辑和查看模式下查询信息。
 */
const initFormData = async (id = "") => {
  if (props.mode === "new") {
    // 新增模式下，初始化表单ID和创建时间
    pageData.formData.createdBy = userStore.userData?.id;
    pageData.formData.createdTime = new Date().toISOString();
  } else if (props.mode === "edit" || props.mode === "view") {
    // 编辑或查看模式下，根据ID查询用户信息
    const resultData = await http(
      "GET",
      `${GET_FORM_PATH}?id=${encodeURIComponent(id)}`
    );
    pageData.formData = resultData.data;
  }
};

/**
 * 角色选择器更改
 */
const handleCodeTypeChange = (value: any) => {
  if (value !== "other") {
    pageData.formData.id = value;
  }
};
/**
 * 提交表单。
 * 根据不同的操作模式（新增、编辑），调用不同的接口保存信息。
 */
const handleSubmit: FormProps["onSubmit"] = async () => {
  const validateResult = await formInstance?.value?.validate();
  if (validateResult === true) {
    let resultData;

    if (props.mode === "view") {
      handleClose();
    }

    if (pageData.formData.id === "") {
      MessagePlugin.error("请选择角色代号！");
      return;
    }

    if (props.mode == "new") {
      // 新增
      resultData = await http("POST", CREATE_FORM_PATH, pageData.formData);
    } else {
      // 更新
      pageData.formData.updatedTime = new Date().toISOString();
      pageData.formData.updatedBy = userStore.userData?.id;

      resultData = await http("POST", UPDATE_FORM_PATH, pageData.formData);
    }

    // 根据接口返回结果，提示操作成功或失败，并关闭对话框
    if (resultData.status === "success") {
      emits("close", "success");
    }
  }
};

const handleClose = () => {
  // 关闭对话框，并返回取消操作标识
  emits("close", "cancel");
};

// 如果是查看模式，设置表单为只读
if (props.mode == "view") {
  pageData.readOnly = true;
}

// 初始化表单数据，根据传入的ID
initFormData(props.id);
</script>
